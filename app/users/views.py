...
# Eliminar un usuario
@swagger_auto_schema(method='delete', responses={204: "Usuario eliminado correctamente"})
@api_view(['DELETE'])
@permission_classes([IsAuthenticated])
...
def obtain_token(request):
    ...
    if not user:
        return Response({"detail": "Usuario no encontrado", "code": "user_not_found"}, status=404)
...
@swagger_auto_schema(
    method='get',
    manual_parameters=[
        openapi.Parameter('username', openapi.IN_PATH, description="Nombre de usuario", type=openapi.TYPE_STRING)
    ],
    responses={
...
def get_ghibli_data(request, username):
    ...
    if not user:
        return Response({"detail": "Usuario no existente"}, status=status.HTTP_404_NOT_FOUND)
    ...
    if user.role == "admin":
        ...
        responses = {}
        for role in roles:
            response = requests.get(base_url.format(role))
            if response.status_code == 200:
                responses[role] = response.json()
            else:
                responses[role] = {"error": "No se pudo obtener datos de {}".format(role)}
        return Response(responses, status=status.HTTP_200_OK)
    else:
        # Si el usuario tiene otro rol, hacer una petición a su respectiva categoría
        response = requests.get(base_url.format(user.role))
        if response.status_code == 200:
            return Response(response.json(), status=status.HTTP_200_OK)
        return Response({"error": "No se pudo obtener datos"}, status=status.HTTP_400_BAD_REQUEST)